          ...
          <div v-else-if="sample" class="space-y-6">
            ...
            >
              <div class="flex flex-col lg:flex-row">
                ...
                <div
                  v-if="sample.sample_image_url"
                  class="lg:w-80 relative group"
                >
                  <div class="aspect-square lg:aspect-auto lg:h-full">
                    jjjj
                    <Image
                      :src="sample.sample_image_url"
                      alt="Water Sample"
                      preview
                    ...
                    >
                      <i class="pi pi-search-plus text-white text-3xl"></i>
                    </div>
                  </div>
                  <!-- Image Badge -->
                  <div
                    class="absolute top-4 right-4 bg-white dark:bg-gray-800 rounded-full px-3 py-1 shadow-lg"
                  >
                    <i class="pi pi-camera text-blue-600 mr-2"></i>
                    <span
                ...
                <div class="flex-1 p-6">
                  ...
                  >
                    <div class="flex items-center gap-6">
                      <div
                        class="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-3xl shadow-lg flex-shrink-0"
                      >
                      ...
                      <div>
                        ...
                        <p
                          class="text-gray-600 dark:text-gray-400 flex items-center gap-2 mt-2"
                        >
                        ...
                        <div class="flex flex-wrap gap-2 mt-3">
                          <Tag
                            :value="formatStatus(sample.quality_status)"
                            :severity="
                              getQualitySeverity(sample.quality_status)
                            "
                          />
                          <Tag
                            :value="sample.risk_level"
                            :severity="getRiskSeverity(sample.risk_level)"
                            icon="pi pi-exclamation-triangle"
                          />
                          <Badge
                            :value="formatStatus(sample.status)"
                            :severity="getStatusBadgeSeverity(sample.status)"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              <h2
                class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6"
              >
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                >
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <i
                      ...
                      v-tooltip.top="'World Health Organization'"
                    ></i>
                  </div>
                  <div
                    class="text-5xl font-bold mb-2"
                    :style="{ color: getWQIColor(sample.wqi_who) }"
                  >
                    {{ sample.wqi_who || "N/A" }}
                  </div>
                  <div
                  ...
                  <ProgressBar
                    :value="Number(sample.wqi_who)"
                    :showValue="false"
                    :pt="{
                      value: {
                        style: `background: ${getWQIColor(sample.wqi_who)}`,
                      },
                    }"
                  />
                </div>
                ...
                >
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <i
                      class="pi pi-info-circle text-gray-400"
                      v-tooltip.top="'National Sanitation Foundation'"
                    ></i>
                  </div>
                  <div
                    class="text-5xl font-bold mb-2"
                    :style="{ color: getWQIColor(sample.wqi_nsf) }"
                  >
                    {{ sample.wqi_nsf || "N/A" }}
                  </div>
                  <div
                  ...
                  <ProgressBar
                    :value="Number(sample.wqi_nsf)"
                    :showValue="false"
                    :pt="{
                      value: {
                        style: `background: ${getWQIColor(sample.wqi_nsf)}`,
                      },
                    }"
                  />
                </div>
                ...
                >
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <i
                      class="pi pi-info-circle text-gray-400"
                      v-tooltip.top="'Canadian Council'"
                    ></i>
                  </div>
                  <div
                    class="text-5xl font-bold mb-2"
                    :style="{ color: getWQIColor(sample.wqi_ccme) }"
                  >
                    {{ sample.wqi_ccme || "N/A" }}
                  </div>
                  <div
                  ...
                  <ProgressBar
                    :value="Number(sample.wqi_ccme)"
                    :showValue="false"
                    :pt="{
                      value: {
                        style: `background: ${getWQIColor(sample.wqi_ccme)}`,
                      },
                    }"
                  />
                </div>
                ...
                <div
                  class="bg-blue-50 dark:bg-blue-900/30 border-2 border-blue-200 dark:border-blue-800 rounded-lg p-6 text-center"
                >
                  <div class="flex justify-between items-center mb-4">
                    <span
                      class="text-sm font-semibold text-blue-700 dark:text-blue-300"
                      >Egyptian Custom</span
                    >
                    <i class="pi pi-star-fill text-blue-500"></i>
                  </div>
                  <div
                    class="text-5xl font-bold mb-2"
                    :style="{ color: getWQIColor(sample.wqi_custom) }"
                  >
                    {{ sample.wqi_custom || "N/A" }}
                  </div>
                  <div
                    class="text-sm font-semibold text-blue-600 dark:text-blue-400 mb-4"
                  >
                    {{ getWQIStatus(sample.wqi_custom) }}
                  </div>
                  <ProgressBar
                    :value="Number(sample.wqi_custom)"
                    :showValue="false"
                    :pt="{
                      value: {
                        style: `background: ${getWQIColor(sample.wqi_custom)}`,
                      },
                    }"
                  />
                </div>
            ...
            <!-- Risk Factors -->
            <div
              v-if="sample.risk_factors && sample.risk_factors.length"
            ...
            >
              ...
              <div class="space-y-3">
                <Message
                  ...
                  :closable="false"
                >
                  <strong>{{ formatParameter(risk.parameter) }}:</strong>
                  {{ risk.message }}
                </Message>
              </div>
            </div>
            ...
            <!-- AI Predictions -->
            <div
              v-if="sample.ai_predictions"
              class="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg shadow-sm p-6"
            >
              <h3
                class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2"
              >
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-4">
                  <span
                    class="text-sm font-semibold text-gray-700 dark:text-gray-300 min-w-32"
                    >Confidence Score:</span
                  >
                  <ProgressBar
                    :value="Number(sample.ai_confidence)"
                    class="flex-1"
                  />
                  <span
                    class="text-sm font-bold text-purple-600 dark:text-purple-400 min-w-16"
                    >{{ sample.ai_confidence }}%</span
                  >
                </div>
                <div
                  v-if="sample.ai_recommendations"
                ...
                <TabPanel header="Chemical">
                  ...
                  >
                    ...
                    <ParameterCard
                      ...
                      :value="sample.electrical_conductivity"
                      unit="µS/cm"
                      icon="pi pi-bolt"
                    />
                    <ParameterCard
                      label="Total Hardness"
                      :value="sample.total_hardness"
                      unit="mg/L"
                      icon="pi pi-shield"
                    />
                    <ParameterCard
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              <h3
                class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-6"
              >
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div class="flex flex-col gap-2">
                  ...
                  >
                  <span
                    class="text-base text-gray-800 dark:text-gray-100 font-semibold"
                    >{{ formatDateTime(sample.collection_date) }}</span
                  >
                </div>
                <div class="flex flex-col gap-2">
                  <span
                  ...
                  <span
                    class="text-base text-gray-800 dark:text-gray-100 font-semibold"
                    >{{ sample.collector?.name || "N/A" }}</span
                  >
                </div>
                <div class="flex flex-col gap-2">
                  <span
                ...
                <div v-if="sample.verified_at" class="flex flex-col gap-2">
                  ...
                  <span
                    class="text-base text-gray-800 dark:text-gray-100 font-semibold"
                    >{{ formatDateTime(sample.verified_at) }}</span
                  >
                </div>
                <div
                  v-if="sample.notes"
...
const downloadReport = async () => {
  ...
  try {
    console.log('Downloading report for sample:', route.params.id);
    
    // استدعاء الـ service مباشرة
    const blob = await samplesService.export({
    ...
    link.download = `water-sample-${route.params.id}-${date}.xlsx`;
